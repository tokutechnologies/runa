...
(function () {
  ...
  fetch("content/posts.json", { cache: "no-store" })
    ...
    .then(function (all) {
      ...
      /* featured = newest — its side panel uses the post's own first image, if any */
      if (cfg.featured && posts.length) {
        var f = posts[0];
        var stamp = cfg.kind === "fiction" ? "Fresh ink" : "Exhibit A";
        var side = f.image
          ? '<img src="' + f.image + '" alt="" loading="lazy"/><span class="st onimg">' + stamp + "</span>"
          : '<span class="st">' + stamp + "</span>";
        feat.innerHTML = '<a class="bfeat" href="read.html?slug=' + f.slug + '" data-t="' + (f.type || "") + '">' +
          ...
          '<div class="side' + (f.image ? " hasimg" : "") + '">' + side + "</div></a>";
      }
